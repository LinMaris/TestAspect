...
//#pragma mark ---------------------------------------------
#pragma mark - 用来保存信息, 存放被hook的实例, 参数, 方法
...
#pragma mark - 用来追踪一个唯一的aspect
// AspectIdentifier对应的实例，里面包含了单个的 Aspect 的具体信息, 包括执行时机，要执行 block 所需要用到的具体信息：包括方法签名、参数等等, 初始化AspectIdentifier的过程实质是把我们传入的block打包成AspectIdentifier。
...
@property (nonatomic, strong) NSMethodSignature *blockSignature;  // 方法签名
@property (nonatomic, weak) id object;
@property (nonatomic, assign) AspectOptions options;
@end

#pragma mark - 是一个用来存储所有的aspect的容器，可能存储实例方法/类方法
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    NSCParameterAssert(selector);
    // 被hook过, 元类, kvo类或者获取动态创建一个子类    ------> hook class 处理
    Class klass = aspect_hookClass(self, error);
    
    // 下面全部是 ----------------------------------------> hook selector 处理
    Method targetMethod = class_getInstanceMethod(klass, selector);
    IMP targetMethodIMP = method_getImplementation(targetMethod);
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {  //当前IMP不是消息转发方法
        // Make a method alias for the existing method implementation, it not already copied.
        // 获取原始方法的selector对应的IMP的方法解码typeEncoding
        ...
        // 把当前的sel方法替换成forwardInvocation方法, selector被执行的时候, 直接会触发消息转发从而进入forwardInvocation
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self);
    // 当self为类对象时, object_getClass(self)返回类对象中的isa指针, 即指向元类对象的指针,[self class] 返回其自身
    // 当self为实例变量时, object_getClass(self)与[self class]返回结果一致, 均获得isa指针, 即指向类对象的指针
	Class statedClass = self.class;  // isa
	Class baseClass = object_getClass(self);  // return self-isa
	NSString *className = NSStringFromClass(baseClass);

    // Already subclassed 当前Class是以xxx_Aspects_后缀结尾的名称，说明这个Class已经被hook过了, 直接返回当前Class，去执行hook selector的过程
	if ([className hasSuffix:AspectsSubclassSuffix]) {
		return baseClass;

	}else if (class_isMetaClass(baseClass)) {
        // We swizzle a class object, not a single object.
        // 是元类, 说明是hook 类的所有实例对应的方法
        return aspect_swizzleClassInPlace((Class)self);
        
    }else if (statedClass != baseClass) {
        // Probably a KVO'ed class. Swizzle in place. Also swizzle meta classes in place.
        // 不相等, 说明是KVO过的对象 isa指针会指向一个中间类
        return aspect_swizzleClassInPlace(baseClass);
    }

    // 运行到这里, 说明没被hook过, 同时不是类的所有实例对应的方法, kvo对象过的方法, 那就是某一个类的实例的对应的方法.
    // Default case. Create dynamic subclass.  默认动态新建一个子类subclass__Aspects_
	...
	if (subclass == nil) {
        ...
        // 把子类系统的ForwardInvocation方法替换成 __ASPECTS_ARE_BEING_CALLED__, 用来消息转发
		aspect_swizzleForwardInvocation(subclass);
        
        // 把subClass的isa指向statedClass
		aspect_hookedGetClass(subclass, statedClass);
        
        // 把subClass的元类isa, 也指向statedClass
